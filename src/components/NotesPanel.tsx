...
type NotesPanelProps = {
  ...
  theme: Theme;
  notes: NoteGroup[];
  verseTags: Record<number, VerseTag[]>;
  onClose: () => void;
};
    ...
    <>
      ...
      >
        <View style={styles.header}>
          <Text style={[styles.title, { color: theme.colors.sectionTitle }]}>All Notes</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          {notes.length === 0 ? (
            <Text style={[styles.emptyText, { color: theme.colors.textMuted }]}>You have no notes yet.</Text>
          ) : (
            notes.map((group) => {
              const tags = verseTags[group.verseId] ?? [];
              return (
                <View
